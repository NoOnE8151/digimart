...
import OnboardRequest from "@/components/payment errors/onboard";
...
const AddProductForm = ({
  ...
  //check if seller has onboarded or not before they add a product to sell.
  const [showShowOnboardError, setShowOnboardError] = useState();
      ...
      const filesToDelete = [
        {
          ...
          resourceType:
            productType === "audio" || productType === "video"
              ? "video"
              : productType === "image"
              ? "image"
              : "raw",
  ...
  if (showShowOnboardError) {
    return (
      <div className="fixed top-0 left-0 right-0 bottom-0 flex justify-center items-center bg-black/50 z-50">
        <OnboardRequest setShowOnboardError={setShowOnboardError} setComponentVisibility={setIsProductAdding} />
      </div>
    );
  }
  return (
    <div className="absolute w-[100vw] min-h-[100vh] flex py-[5rem] justify-center">
      <form
        onSubmit={handleSubmitProduct(addProduct)}
        className="bg-background md:border-[1px] md:border-gray-400 absolute md:w-[45%] p-7 rounded-lg flex flex-col gap-5 "
      >
        ...
        <div className="md:w-[92%] flex flex-col gap-7">
            ...
            <textarea
              ...
              {...registerProduct("description", {
                ...
                minLength: {
                  value: 20,
                  message:
                    "description lenth should be minimum 20 characters long",
                },
                maxLength: {
                  value: 500,
                  message: "maximum length can be 500 characters",
                },
              })}
            ></textarea>
          ...
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              className={`capitalize border-[1px] border-black rounded-sm px-5 py-2 flex items-center justify-between font-semibold ${
                product.url ? "cursor-not-allowed" : "cursor-pointer"
              }`}
            >
              {productType}{" "}
          ...
          <div className="flex flex-col gap-2">
            ...
            {product.url && (
              <div className="flex items-center justify-between border-[1px] border-border px-3 gap-5 py-3 rounded-lg relative max-h-26">
                <div className="w-[15%]">
                  <img
                    src={
                      productType === "document"
                        ? "/assets/product icons/document.jpg"
                        : productType === "video"
                        ? "/assets/product icons/video.jpg"
                        : productType === "audio"
                        ? "/assets/product icons/audio.jpg"
                        : "/assets/product icons/image.jpg"
                    }
                    alt="thumbnail"
                ...
                <button
                  ...
                  type="button"
                  className="w-[5%]flex justify-center h-full cursor-pointer"
                >
                  <X />
                </button>
              </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="flex items-center gap-5">
              <button
                ...
                type="button"
                className={`flex flex-col items-center justify-center border-foreground/50 border-2 ${
                  thumbnail.url ? "" : "border-dashed"
                } md:w-[20%] w-[30%] h-[5rem] rounded-lg cursor-pointer hover:bg-input`}
              >
                {!isThumbnailUploading && !thumbnail.url && (
                ...
                {isThumbnailUploading && (
                  <div className="bg-black/70 w-full h-full text-white flex flex-col justify-center items-center rounded-lg">
                    <ArrowUpFromLine
                      size={24}
                      strokeWidth={1.5}
                      className="fade-in-out"
                ...
                {thumbnail.url && !isThumbnailUploading && (
                  <div className="group w-full h-full relative">
                    ...
                    <div className="absolute inset-0 flex justify-center items-center bg-black/0 group-hover:bg-black/20">
                      <span className="opacity-0 group-hover:opacity-100 text-muted">
                        Change
                      </span>
                    </div>
                  </div>
          ...
          <div className="flex flex-col gap-3 w-full">
            ...
            {categories.length > 0 && (
              <ul className="list-none flex items-center flex-wrap gap-3">
                {categories.map((category, idx) => {
                  return (
                    <li
                      key={idx}
                      className="flex items-center justify-between gap-2 shadow-2xs border-border border-[1px] px-3 py-1 rounded-full"
                    >
                      <span>{category}</span>{" "}
                      <button
            ...
            />
            {categoryError && (
              <p className="text-red-500">this keyword is already added</p>
            )}
            <button
              type="button"
          ...
          <input
            ...
            {...registerProduct("checkbox", {
              required: {
                value: true,
                message:
                  "you must accept terms and services to sell this product.",
              },
            })}
          />
        ...
        {productErrors.checkbox && (
          <p className="text-red-500">{productErrors.checkbox.message}</p>
        )}
        ...
        <input
          ...
          className={`py-2 px-5 rounded-full font-semibold text-white text-lg  ${
            isUploading || isThumbnailUploading
              ? "cursor-not-allowed bg-element/70"
              : "cursor-pointer bg-element "
          }`}
        />
      </form>
    </div>
